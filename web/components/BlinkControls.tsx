...
export default function BlinkControls({ brightness, onMs, offMs, count, busy, ready, blinking, onBrightness, onOnMs, onOffMs, onCount, onStart, onStop }: Props) {
  ...
  return <div className="blink-section">
    <div className="panel-intro"><p>เลือกจังหวะสำเร็จรูป หรือกำหนดเวลาติด/ดับเอง</p><span className={`blink-state ${blinking ? 'blink-state-active' : ''}`}><Radio size={13} />{blinking ? 'กำลังกระพริบ' : 'พร้อมใช้งาน'}</span></div>
