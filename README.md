...
Important free-tier limits: Render Free has 0.1 CPU and 512 MB RAM, sleeps after 15 minutes of inactivity, and loses local SQLite data on sleep/restart/redeploy. The Blueprint therefore writes SQLite to `/tmp/luma.sqlite` for a live classroom demonstration, **not durable storage**; saved presets and audit history can disappear. Fresh neural TTS exceeded even a 120-second timeout in a 0.1-CPU container test, so the Render image precomputes 308 common Thai light replies at build time. A cache miss returns `503` with `code: "tts_cache_miss"`; the main frontend falls back to the browser's Thai voice, whose quality and availability depend on the device. The frontend can stay online while the API sleeps, but the first API request may take about a minute. Tailscale's free Personal plan also has an ephemeral-resource usage allowance. Confirm speech on the live Render deployment before claiming the audio issue resolved.
